...
function AdvancedEditor() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (collabProvider && collabProvider.document && collabProvider.awareness) {
        ...
        loadDocument()
        // setupSocket() // This function is removed, so this line is removed.
      } else {
  ...
  // Add this useEffect for real-time Yjs socket sync
  useEffect(() => {
    ...
    const handleYjsSync = ({ documentId, update }) => {
      if (documentId === docId) {
        Y.applyUpdate(provider.document, new Uint8Array(update));
      }
    };

    const handleYjsUpdate = ({ documentId, update }) => {
      if (documentId === docId) {
        Y.applyUpdate(provider.document, new Uint8Array(update));
      }
    };

    socketService.onYjsSync(handleYjsSync);
    socketService.onYjsUpdate(handleYjsUpdate);

    // Observe local Y.Doc updates and send to backend
    const updateHandler = (update) => {
      socketService.sendYjsUpdate(docId, update);
    };
    provider.document.on('update', updateHandler);
    ...
    // Clean up listeners on unmount or provider/docId change
    return () => {
      provider.document.off('update', updateHandler);
      // Optionally remove socket listeners if your socketService supports it
    };
  }, [provider, docId]);
  ...
  // Remove setupSocket and its call from useEffect
